globals [
  ...
  buildingtemp ;; Building temperature
  gtempc
  btempc
  airtempc
]
...
to setup
  ...
  set-default-shape radiators "square"
  setup-world
  reset-ticks
  setup-thermometer
  set buildingtemp 20
...
to setup-world
  set sky-top max-pycor - 1
  set earth-top min-pycor + 1
  ask patches [  ;; set colors for the different sections of the world
    if pycor > sky-top [  ;; space
      set pcolor scale-color white pycor 3 0
    ]
    if pycor <= sky-top and pycor > earth-top [ ;; sky
      set pcolor scale-color blue pycor -10 3
    ]
    if pycor < earth-top
...
to setup-thermometer
  ask patches [
    ; create the thermometer
    if (((abs pxcor) <= 10) and
        (pycor < (max-pycor - 4)) and
        (pycor > (min-pycor + 2)))
      [ set pcolor cyan ]
  ]
end
...
;;to change-sun-angle
;;if ticks > 2000
  ;;[create-rays sun-brightness [
    ;;  set heading 160 + 10
      ;;set color yellow
      ;; rays only come from a small area
      ;; near the top of the world
      ;;setxy (random 20) + min-pxcor max-pycor
  ;;] ]
;;end

;; ====================================
;; To add Environment features
;; ====================================
to add-tree
  if mouse-down?
    [create-trees 1
...
to encourter-building
  ;; rays encounter top of building from topleft
  ask rays[
    if (patch-right-and-ahead (180 - heading) 0.2 != nobody) and (heading > 90) and (heading < 180)[
      ifelse [pcolor] of patch-right-and-ahead (180 - heading) 0.2 = yellow
    [
    ifelse 100 * b-albedo > random 100
      [ set heading 180 - heading  ] ;; reflect
      [ rt random (270 - heading)  ;; absorb into the earth
        set color pink
        set breed heats
        ]
  ]
  ...
  ;; rays encounter top of building from topright
  ask rays[
    if (patch-left-and-ahead (heading - 180) 0.2 != nobody) and (heading > 180) and (heading < 270)[
      ifelse [pcolor] of patch-left-and-ahead (heading - 180) 0.2 = yellow
      [
    ifelse 100 * b-albedo > random 100
      [ set heading 180 - heading  ] ;; reflect
      [ lt random (heading - 90)  ;; absorb into the earth
        set color pink
        set breed heats
       ]
  ]
  ...
  ;; rays encounter left side of building from topleft
  ask rays[
    if (patch-left-and-ahead (heading - 90) 0.2 != nobody) and (heading > 90) and (heading < 180)[
      ifelse [pcolor] of patch-left-and-ahead (heading - 90) 0.2 = yellow
    [
    ifelse 100 * b-albedo > random 100
      [ set heading 360 - heading  ] ;; reflect
      [ lt random (heading)  ;; absorb into the earth
        set color pink
        set breed heats
        ]
  ]
  ...
  ;; rays encounter left side of building from bottom left
  ask rays[
      if (patch-right-and-ahead (90 - heading) 0.2 != nobody) and (heading > 0) and (heading < 90)[
    ifelse [pcolor] of patch-right-and-ahead (90 - heading) 0.2 = yellow
    [
    ifelse 100 * b-albedo > random 100
      [ set heading 360 - heading  ] ;; reflect
      [ rt random (180 - heading) ;; absorb into the earth
        set color pink
        set breed heats
       ]
  ]
      ...
      ;; rays encounter right side of bulding from top right
   ask rays[
    if (patch-right-and-ahead (270 - heading) 0.2 != nobody) and (heading > 180) and (heading < 270)[
      ifelse [pcolor] of patch-right-and-ahead (270 - heading) 0.2 = yellow
    [
    ifelse 100 * b-albedo > random 100
      [ set heading 360 - heading  ] ;; reflect
      [ rt random (360 - heading)  ;; absorb into the earth
        set color pink
        set breed heats
       ]
  ]
    ...
    ;; rays encounter right side of building from bottom right
    ask rays[
      if (patch-left-and-ahead (heading - 270) 0.2 != nobody) and (heading > 270) and (heading < 360)[
      ifelse [pcolor] of patch-left-and-ahead (heading - 270) 0.2 = yellow
    [
    ifelse 100 * b-albedo > random 100
      [ set heading 360 - heading  ] ;; reflect
      [ lt random (heading - 180) ;; absorb into the earth
        set color pink
        set breed heats
       ]
  ]
  ...
  ask rays[
    if (patch-ahead 1 != nobody) and (heading = 180) [
    ifelse [pcolor] of patch-ahead 0.2 = yellow
    [ifelse 100 * b-albedo > random 100
      [set heading heading - 180]
      [ set heading random 100 + random 160
        set color pink
        set breed heats]]
    [set heading heading]
  ]
...
to trap-buildingheat
  ifelse count patches with [pcolor = yellow] > 0
  [set buildingtemp 20 + (count heats with [color = pink] / count patches with [pcolor = yellow]) * 2 + 273]
  [set buildingtemp 20]
  ask heats with [color = pink ][
    let dist 0.5 * random-float 0.5
    ifelse can-move? dist
      [ fd dist ]
      [ die ]
    ;; heat approaching left side of building from topright
    if(patch-right-and-ahead (270 - heading) 1 != nobody) and pcolor = yellow and (heading > 180) and (heading < 270) and (ycor > earth-top + 1) [
      ifelse [pcolor] of patch-right-and-ahead (270 - heading) 1 != yellow
      [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
     ...
     ;; heat approaching left side of building from bottomright
    if(patch-left-and-ahead (heading - 270) 1 != nobody) and pcolor = yellow and (heading > 270) and (heading < 360)[
      ifelse [pcolor] of patch-left-and-ahead (heading - 270) 1 != yellow
      [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
        [ set breed IRs                    ;; let some escape as IR
          set heading heading
          set color magenta ]
        [set heading heading - 180]]
        [ fd dist]]
       ;; heat approaching right side of building from topleft
    if(patch-left-and-ahead (heading - 90) 1 != nobody) and pcolor = yellow and (heading > 90) and (heading < 180) and (ycor > earth-top + 1) [
      ifelse [pcolor] of patch-left-and-ahead (heading - 90) 1 != yellow
   [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
        [ set breed IRs                    ;; let some escape as IR
          set heading heading
          set color magenta ]
        [set heading heading - 180]]
        [ fd dist]]
      ;; heat approaching right side of building from bottomleft
    if(patch-right-and-ahead (90 - heading) 1 != nobody) and pcolor = yellow and (heading > 0) and (heading < 90)[
      ifelse [pcolor] of patch-right-and-ahead (90 - heading) 1 != yellow
       [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
        [ set breed IRs                    ;; let some escape as IR
          set heading heading
          set color magenta ]
        [set heading heading - 180]]
        [ fd dist]]
    ;; heat approaching perpendicularly to wall of building
    if(patch-ahead 1 != nobody) and pcolor = yellow and (ycor > earth-top + 1) [
      ifelse [pcolor] of patch-ahead 1 != yellow
       [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
        ...
        [set heading heading - 180]]
        [ fd dist]]
         ;; heat approaching top side of building from bottomright
    if(patch-right-and-ahead (360 - heading) 1 != nobody) and pcolor = yellow and (heading > 270) and (heading < 360)[
      ifelse [pcolor] of patch-right-and-ahead (360 - heading) 1 != yellow
       [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
        [ set breed IRs                    ;; let some escape as IR
          set heading heading
          set color magenta ]
        [set heading heading - 180]]
        [ fd dist]]
    ;; heat approaching top side of building from bottomleft
    if(patch-left-and-ahead (heading) 1 != nobody) and pcolor = yellow and (heading > 0) and (heading < 90)[
      ifelse [pcolor] of patch-left-and-ahead (heading) 1 != yellow
   [ifelse (buildingtemp ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * bemissivity * count patches with [pcolor = yellow] > 0
        [ set breed IRs                    ;; let some escape as IR
          set heading heading
          set color magenta ]
    ...
    if ycor <= earth-top + 1 [set heading heading - 180]
...
to run-heat    ;; advances the heat energy turtles
  ;; the temperature is related to the number of heat turtles by the following formula. How can we modify this
  ;; to the number of heat turtles in the thermometer instead?
  ;; the temperature that we are measuring here is the temperature on the ground, not on the air above.
  set temperature 20 + (count heats with [color = red] / ((max-pxcor - min-pxcor) * (earth-top - min-pycor)) * 2) + 273
  ask heats with [color = red ]
  [
    let dist 0.5 * random-float 0.5
    ifelse can-move? dist
      [ fd dist ]
      ...
      [die] ] ;; if we're hitting the edge of the world, turn around
    if ycor >= earth-top [  ;; if heading back into sky

      if(patch-right-and-ahead (360 - heading) 1 != nobody) and pcolor != yellow and (heading > 270) and (heading < 360) [
        if [pcolor] of patch-right-and-ahead (360 - heading) 1 != yellow
         [ifelse (temperature ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * gemissivity * (max-pxcor - min-pxcor) >
            random (((temperature ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * (max-pxcor - min-pxcor)))
        [ set breed IRs                    ;; let some escape as IR
          set heading 0
          set color magenta ]
   ...
   if(patch-left-and-ahead (heading) 1 != nobody) and pcolor != yellow and (heading > 0) and (heading < 90) [
        if [pcolor] of patch-left-and-ahead (heading) 1 != yellow
         [ifelse (temperature ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * gemissivity * (max-pxcor - min-pxcor) >
            random (((temperature ^ 4 - airtemp ^ 4) * (5.67 * 10 ^ -8) * (max-pxcor - min-pxcor)))
        [ set breed IRs                    ;; let some escape as IR
          set heading 0
   ...
   if(patch-ahead 1 != nobody) and pcolor != yellow and (heading = 0)[
      ...
      if (patch-ahead 1 != nobody) and (heading = 0)[
        if [pcolor] of patch-ahead 1 = yellow [
        ifelse buildingtemp < temperature
        [set heading 0
         set color pink
        ]
        [set heading heading - 180]]
      ...
      if (patch-right-and-ahead (360 - heading) 1 != nobody) and (heading > 270) and (heading < 360)[
        if [pcolor] of patch-right-and-ahead (360 - heading) 1 = yellow [
        ifelse buildingtemp < temperature
        [set heading 0
         set color pink
        ...
        [set heading 540 - heading ]]
         ...
         if (patch-left-and-ahead heading 1 != nobody) and (heading > 0) and (heading < 90)[
        if [pcolor] of patch-left-and-ahead heading 1 = yellow [
        ifelse buildingtemp < temperature
        [set heading 0
         set color pink
...
;; ================================================
to atm-absorb
  ask rays with [(ycor < (max-pycor - 4)) and (ycor > (min-pycor + 2 ))][
  ifelse 100 * absorption > random 100
    [ rt (random 360)
      set color orange
      set breed heats
    ]
...
to trap-heat
  ...
  [let dist 0.5 * random-float 1
    ...
    [set heading heading - 180]
    if ycor = (max-pycor - 5)[
      ifelse airtemp > 18 + random 5
      [set breed IRs
       set heading 80 - random 160
       set color magenta]
    ...
    ;; to trap heat when encounter building
     ;; rays encounter top of building from topleft
    ask heats with [color = orange][
    if (patch-right-and-ahead (180 - heading) 0.2 != nobody) and (heading > 90) and (heading < 180)[
      ifelse [pcolor] of patch-right-and-ahead (180 - heading) 0.2 = yellow
    ...
    if ycor >= max-pycor [die] ;; heat escape into upper atmosphere
  ]
...
to run-IR
  ...
  ask IRs with [(ycor < (max-pycor - 4)) and (ycor > (min-pycor + 2 ))][
  ifelse 100 * absorption * 50 > random 100 ;; Re-absorption by the atmosphere
    [ rt (random 360)
      set color orange
      set breed heats
    ]
...
;; =========================================================
to take-airtemp
  set airtemp 20 + (((count heats with [pcolor = cyan]) / count patches with [pcolor = cyan]) * 10) + 273
end

to measure-temp
  set gtempc temperature - 273
  set btempc buildingtemp - 273
  set airtempc airtemp - 273
end
...
to add-heat
  if count radiators > 0
  [create-heats 1
    [
    set color orange
      move-to one-of radiators
  ]
  ]
end
...
"" ""
PENS
"air temp" 1.0 0 -7500403 true "" "plot airtempc"
"ground temp" 1.0 0 -2674135 true "" "plot gtempc"
